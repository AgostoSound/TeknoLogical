...
inline float equalPowerAttenuation(float pan) {
...
struct LowPassFilter {
	...
	void setCutoff(float cutoff, float sampleRate, float q = 0.707f) {
		cutoff = std::max(20.f, std::min(cutoff, 0.45f * sampleRate));
...
inline float mapLP_Cutoff(float filterParam, float sampleRate) {
	(void) sampleRate;
	return std::pow(10.f, rescale(filterParam, -10.f, 0.f, std::log10(20.f), std::log10(20000.f)));
}

inline float mapHP_Cutoff(float filterParam, float sampleRate) {
	(void) sampleRate;
	...
	float process(float input, float filterParam, float sampleRate) {
		if (filterParam < 0.f) {
			if (filterParam != lastParam || sampleRate != lastSampleRate) {
				...
				float q = mapResonanceQ((-filterParam) / 10.f);
...
inline float applyVolume(float signal, float volumeParam) {
	return signal * clamp(volumeParam / 10.f, 0.f, 1.f);
}
...
inline float applyBoost(float signal, float push) {
	if (push == 1.f)
		return clamp(signal * 1.5f, -1.f, 1.f);
	return signal;
}
...
inline void panMonoEqualPower(float in, float pan, float& l, float& r) {
	float gl = 0.f;
	float gr = 0.f;
	equalPowerGains(pan, gl, gr);
	l += in * gl;
	r += in * gr;
}

inline void balanceStereoEqualPower(float& l, float& r, float pan) {
